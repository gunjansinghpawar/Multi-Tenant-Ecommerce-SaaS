...
## 1. Purpose & Overview
Toggle experimental features platform-wide or per-tenant.

## 2. User Flow & Navigation
- **Entry Point**: Sidebar > Feature Flags
...
- **Page Header**: Distinct Title, Breadcrumb (Super Admin > Feature Flags).
- **Layout Type**: Custom SaaS Layout optimized for high-density data.
- **Widgets / Sections**: Focuses on Flag Switch List.

## 4. Key UI Components
- **Core Elements**: Flag Switch List, Rollout Percentage Slider
...
- **Granular Permissions**: `super_admin:feature_flags:read`, `super_admin:feature_flags:write`
