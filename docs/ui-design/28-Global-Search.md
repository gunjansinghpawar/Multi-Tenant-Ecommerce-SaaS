...
## 1. Purpose & Overview
Command palette to instantly find any tenant, user, or setting.

## 2. User Flow & Navigation
- **Entry Point**: Sidebar > Global Search
...
- **Page Header**: Distinct Title, Breadcrumb (Super Admin > Global Search).
- **Layout Type**: Custom SaaS Layout optimized for high-density data.
- **Widgets / Sections**: Focuses on Cmd+K Overlay Modal.

## 4. Key UI Components
- **Core Elements**: Cmd+K Overlay Modal, Categorized Results
...
- **Granular Permissions**: `super_admin:global_search:read`, `super_admin:global_search:write`
