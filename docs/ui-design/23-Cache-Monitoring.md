...
## 1. Purpose & Overview
Redis hit/miss ratios and memory eviction stats.

## 2. User Flow & Navigation
- **Entry Point**: Sidebar > Cache Monitoring
...
- **Page Header**: Distinct Title, Breadcrumb (Super Admin > Cache Monitoring).
- **Layout Type**: Custom SaaS Layout optimized for high-density data.
- **Widgets / Sections**: Focuses on Hit Rate Pie Chart.

## 4. Key UI Components
- **Core Elements**: Hit Rate Pie Chart
...
- **Granular Permissions**: `super_admin:cache_monitoring:read`, `super_admin:cache_monitoring:write`
